module DoubleDouble
  describe DebitAmount do
    ...
    it "should be sensitive to 'context' when calculating balances, if supplied" do
      ...
      expect(@cash.debits_balance({context: @job})).to eq(123 + 321)
      expect(@cash.debits_balance({context: @po})).to  eq(275)
      expect(@cash.debits_balance).to                  eq(123 + 321 + 275 + 999)
      Entry.create!(
          description: 'Foobar5',
      ...
      expect(@cash.debits_balance({context: @po, subcontext: @item_foo})).to eq(123 + 222)
      expect(@cash.debits_balance({context: @po, subcontext: @item_bar})).to eq(1)
